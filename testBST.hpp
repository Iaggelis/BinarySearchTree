...
private:
    ...
    void addHelper(std::shared_ptr<Node> &root, const T &val)
    {
        ...
        current = std::make_unique<Node>(val);
    ...
    bool deleteValueHelper(std::unique_ptr<Node> parent, std::unique_ptr<Node> current, T val)
    {
        if (!current)
        {
            return false;
    ...
    void printMaxPathHelper(std::unique_ptr<Node> &x)
    {
        if (!x)
        {
            return;
    ...
    void traversal_inorder_helper(std::unique_ptr<Node> &x)
    {
        if (x == nullptr)
        {
            return;
...
public:
    ...
    void traversal_inorder_iterative()
    {
        ...
        std::stack<std::unique_ptr<Node>> traversal;
